	...
	Game.prototype = {
		...
		start: function() {
			...
			if (population.length > 0) {
				...
				this.balls   = [ new Ball() ];
				this.paddles = this.population.map(agent => new Paddle({ type: 'auto' }));
		...
		restart: function(learn) {
			...
			if (learn === true) {
				...
				this.paddles.forEach((paddle, p) => {
					...
					let closest_ball = _get_closest_ball.call(this.balls, paddle);

					if (agent !== null && closest_ball !== null) {

						// TODO: Agent needs to learn from ball position
						// for both success and failure scenarios

						// TODO: Remember training data
				...
				this.training.forEach((knowledge, k) => {

					this.population.forEach((agent, a) => {
		...
		update: function() {
			...
			for (let p = 0, pl = paddles.length; p < pl; p++) {
				...
				if (agent !== null && closest_ball !== null) {
					...
					// TODO: Use decisions done by Agent
					// in order to influence the Paddle's
					// movement. Use a threshold in order
					// to filter out noise from signal
